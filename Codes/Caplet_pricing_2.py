...
# ============================================================================
# HULL-WHITE ANALYTICAL CAPLET PRICING
# ============================================================================
...
def HW_ZCB_CallPutOption(CP, K, lambd, eta, P0T, T1, T2):
    """Price a call/put option on a zero-coupon bond"""
    B_r = HW_B(lambd, eta, T1, T2)
    A_r = HW_A(lambd, eta, P0T, T1, T2)
...
def HW_CapletPrice(N, K, lambd, eta, P0T, T1, T2):
    """
    Analytical Hull-White caplet price.
    
    Caplet payoff: N * tau * max(L(T1, T2) - K, 0) paid at T2
    
    Using the relationship between caplet and ZCB put option:
    Caplet = N * (1 + tau*K) * P_put(T1, T2, 1/(1+tau*K))
    
    where P_put is a put option on ZCB P(T1,T2) with strike 1/(1+tau*K)
    """
    tau = T2 - T1
    N_new = N * (1.0 + tau*K)
    K_new = 1.0 / (1.0 + tau*K)
    caplet = N_new * HW_ZCB_CallPutOption('PUT', K_new, lambd, eta, P0T, T1, T2)
    return caplet

# ============================================================================
# MONTE CARLO CAPLET PRICING WITH HULL-WHITE
# ============================================================================

def monte_carlo_caplet_hull_white(N, K, tau, T1, T2, P0T, lambd, eta, n_sims, n_steps=500, verbose=False):
    """
...
start = time.time()
analytical_price = HW_CapletPrice(N, K, lambd, eta, P0T, T1, T2)
...
for n_sims in simulation_sizes:
    ...
    start = time.time()
    mc_price = monte_carlo_caplet_hull_white(N, K, tau, T1, T2, P0T, lambd, eta, n_sims, verbose=(n_sims==10000))
...
for n_sims in simulation_sizes:
    np.random.seed(42)
    mc_price = monte_carlo_caplet_hull_white(N, K, tau, T1, T2, P0T, lambd, eta, n_sims)
...
print("=" * 80)
print("\nNote: Both methods use the Hull-White model with the same parameters.")
print("      Monte Carlo simulates the full dynamics under Q measure.")
